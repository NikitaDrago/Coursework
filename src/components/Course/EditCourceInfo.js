...
const EditCourceInfo = ({lectures, selectLecture, onOption, newTitle, newWeeks, newDescription, isError, setIsError}) =>
  <>
    ...
    <input ref={newTitle} placeholder={'course.name'}
           className={(isError ? 'input_negative ' : '') + "input CourseInfo-container__input"}
           onChange={() => isError && setIsError(false)}
    />
    <input ref={newWeeks} placeholder={`Кол-во недель обучения: `}
           className={(isError ? 'input_negative ' : '') + "input CourseInfo-container__input"}
           onChange={() => isError && setIsError(false)}
    />
    <textarea ref={newDescription} placeholder="Информация о курсе"
    ...
    <UsersList users={lectures} onOption={onOption} profile={selectLecture} isError={isError}/>
  </>;
